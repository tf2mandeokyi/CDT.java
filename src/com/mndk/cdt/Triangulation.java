...
public class Triangulation {
	 ...
	 * @param nRandSamples
	 */
	public Triangulation(FindingClosestPoint closestPtMode, int nRandSamples) {
	...
	private int walkTriangles(int startVertex, V2d pos) {
	    ...
	    while(!found) {
	        ...
	        for(int i_ = 0; i_ < 3; ++i_) {
	            ...
	            V2d vEnd = vertices.get(t.vertices.get(CDTUtils.ccw(i))).pos;
	...
	// int nearestVertexRtree(V2d pos);
	...
	private int findDelaunayPoint(int ia, int ib, List<Integer> points) {
	    ...
	    V2d a = vertices.get(ia).pos;
	    V2d b = vertices.get(ib).pos;
	    int ic = points.get(0);
	    V2d c = vertices.get(ic).pos;
	    for(int it : points) {
	        V2d v = vertices.get(it).pos;
